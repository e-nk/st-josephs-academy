...
export async function GET(
  ...
  try {
    ...
    const summary = {
      ...
      paidInFull: classReport.filter(s => s.status === 'PAID_FULL').length,
      partialPayment: classReport.filter(s => s.status === 'PARTIAL').length,
      unpaid: classReport.filter(s => s.status === 'UNPAID').length,
